...
<div class="row">
	<div class="span12">
		...
		foreach ($list as $row) {
			...
			$this->table->add_row($row->variable_code,$row->variable_text,anchor($link_status.$row->variable_id,$status,'class="btn '.$color.' " title="Change Status"'),'<div class="btn-group">'.anchor($link_detail.$row->variable_id,'<i class="icon-list icon-large"></i>','class="btn" title="Detail"').anchor($link_edit.$row->variable_id,'<i class="icon-pencil icon-large"></i>','class="btn" title="Edit"').'</div>');
		}
